"""Page 5 — 财务分析（重定向至个股研究）

Financial Analysis has been merged into Page 4 (Equity Research) as a tab.
...
st.set_page_config(page_title="Financials → Equity", page_icon="📊", layout="wide")
apply_theme()
render_sidebar()
page_header()
...
st.markdown(
    ...
    f'<div style="font-size:1.05rem;color:{_t0};margin-bottom:8px;">{t("redirect_financial")}</div>'
    f'<div style="font-size:0.85rem;color:{_t1};">'
    f'{"请切换至「个股研究」页面的「财务分析」标签页。" if st.session_state.get("language","en")=="zh" else "Please switch to the Financials tab inside Equity Research."}'
    f'</div>'
    f'</div>',
