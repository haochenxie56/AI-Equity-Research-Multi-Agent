...
def _theme_ret_cell(v, dark: bool) -> str:
    ...
    hdr_c = "#8b949e"
    if v is None:
        return f"<td style='text-align:right;color:{hdr_c}'>N/A</td>"
    c = "#3fb950" if v >= 0 else "#f85149"
    return f"<td style='text-align:right;color:{c}'>{v:+.1f}%</td>"


def _render_theme_ai(ai: dict, lang: str) -> None:
    """Render the analyze_theme_basket() output inside a theme expander."""
    if not ai:
        return
    stage = ai.get("narrative_stage") or "—"
    rows = [
        (t("theme_ai_narrative_stage"), stage),
        (t("theme_ai_macro_alignment"), bi(ai, "macro_alignment", lang)),
        (t("theme_ai_catalysts"),       bi(ai, "key_catalysts", lang)),
        (t("theme_ai_risks"),           bi(ai, "risk_factors", lang)),
        (t("theme_ai_horizon"),         bi(ai, "horizon_bias", lang)),
        (t("theme_ai_summary"),         bi(ai, "summary", lang)),
    ]
    if not any(val for _, val in rows[1:]) and stage == "—":
        st.caption("⚠️ " + t("theme_ai_unavailable"))
        return
...
def _render_market_themes(lang: str, dark: bool) -> None:
    """Market Themes tab: cross-GICS theme momentum heatmap, per-theme drilldown,
    and Scanner universe hand-off. Fixture-fallback / fail-closed throughout."""
    ...
    with st.spinner(t("theme_loading")):
        themes = compute_all_themes("3mo")

    if not themes:
        st.error(t("theme_no_data"))
        return

    txt_c = "#e6edf3" if dark else "#1f2328"
    hdr_c = "#8b949e"
    bd    = "rgba(255,255,255,0.06)" if dark else "rgba(0,0,0,0.06)"
    src_map = {
        "etf":          t("theme_src_etf"),
        "equal_weight": t("theme_src_equal_weight"),
        "fixture":      t("theme_src_fixture"),
    }
    ...
    head_html = "".join(
        f"<th style='padding:6px 12px;text-align:left;color:{hdr_c};"
        f"font-weight:normal;border-bottom:1px solid {bd}'>{h}</th>"
        for h in headers
    )
    body_rows = []
    for r in themes:
        ...
        # Phase 7B Task 2 — stage label (+ confirmed/unconfirmed) and breadth.
        _stage = getattr(r, "stage", "") or ""
        _stage_txt = f"{t('stage_' + _stage)}" if _stage else "—"
        if _stage and not getattr(r, "stage_confirmed", False):
            _stage_txt += f" ({t('theme_stage_unconfirmed')})"
        _bb = getattr(r, "breadth_beat_pct", None)
        _breadth_txt = f"{int(_bb*100)}%" if _bb is not None else "—"
        body_rows.append(
            ...
            f"<td style='text-align:right;color:{txt_c}'>{_breadth_txt}</td>"
            f"<td style='padding:6px 12px;color:{hdr_c}'>"
            f"{src_map.get(r.data_source, r.data_source)}</td></tr>"
        )
    st.markdown(
    ...
    # ── Scanner universe hand-off (review-only; no execution) ─────────────────
    _c1, _c2 = st.columns(2)
    with _c1:
        if st.button(t("theme_send_top_btn"), key="theme_send_top", type="primary"):
            top = send_top_theme_to_scanner(themes, st.session_state, lang)
            if top is not None:
                # Carry the divergence stage so Scanner-side context knows
                # rotating_in vs crowded (Phase 7B Task 2).
                st.session_state["theme_universe_stage"] = getattr(top, "stage", "")
                st.session_state["theme_universe_stage_confirmed"] = bool(
                    getattr(top, "stage_confirmed", False))
                _lbl = top.label_zh if lang == "zh" else top.label_en
                st.success(
                    f"{t('theme_sent_top')}: {_lbl} "
                    f"({len(top.constituents)} {t('theme_tickers')})"
                )
    with _c2:
        if st.button(t("theme_send_all_btn"), key="theme_send_all"):
            _all = send_all_themes_to_scanner(themes, st.session_state, lang)
            st.success(f"{t('theme_sent_all')} ({len(_all)} {t('theme_tickers')})")
...
# ══════════════════════════════════════════════════════════════════════════════
# Tabs — Sector Analysis (existing) + Market Themes (new, cross-GICS)
# ══════════════════════════════════════════════════════════════════════════════
_tab_sector, _tab_themes = st.tabs([t("p2_tab_sector"), t("theme_tab")])

with _tab_themes:
    _render_market_themes(_lang, _dark)
...
with _tab_sector:
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    # SECTION 1B — Offense / Defense reading (Phase 7B Task 2 — outer ring)
    # A multi-window relative-strength differential between the offense and defense
    # baskets vs SPY. This is the GICS-layer "market character" reading the Cockpit
    # takes as a conclusion and the fragility layer consumes as input (e).
    # ══════════════════════════════════════════════════════════════════════════════
    with st.expander(t("p2_od_panel"), expanded=False):
        try:
            from rotation import compute_offense_defense
            _od = compute_offense_defense()
        except Exception:  # noqa: BLE001 — fail-closed; panel simply degrades
            _od = None
        if not _od or _od.get("n_windows", 0) == 0:
            st.caption(t("p2_od_unavailable"))
        else:
            _dir = _od["direction"]
            _od_color = {"offense": "#3fb950", "defense": "#f85149",
                         "balanced": "#d29922"}.get(_dir, "#8b949e")
            st.markdown(
                f"**{t('p2_od_reading')}**: "
                f"<span style='color:{_od_color};font-weight:bold'>"
                f"{t('p2_od_' + _dir)}</span> · {_od['magnitude']} "
                f"(avg {_od['avg_diff']:+.1f}pp · {len(_od['confirming_windows'])}"
                f"/{_od['n_windows']} {t('p2_od_windows_confirm')})",
                unsafe_allow_html=True,
            )
            _bw = _od.get("by_window", {})
            _od_rows = "".join(
                f"<tr><td style='padding:4px 10px'>{w}</td>"
                f"<td style='padding:4px 10px;text-align:right'>{_bw[w]['offense']:+.1f}</td>"
                f"<td style='padding:4px 10px;text-align:right'>{_bw[w]['defense']:+.1f}</td>"
                f"<td style='padding:4px 10px;text-align:right;font-weight:bold'>"
                f"{_bw[w]['diff']:+.1f}</td></tr>"
                for w in ("5d", "10d", "1m", "3m", "6m", "12m") if w in _bw
            )
            st.markdown(
                "<table style='font-size:13px;border-collapse:collapse'>"
                f"<thead><tr><th style='padding:4px 10px;text-align:left'>"
                f"{t('p2_od_window')}</th>"
                f"<th style='padding:4px 10px'>{t('p2_od_offense')}</th>"
                f"<th style='padding:4px 10px'>{t('p2_od_defense')}</th>"
                f"<th style='padding:4px 10px'>{t('p2_od_diff')}</th></tr></thead>"
                f"<tbody>{_od_rows}</tbody></table>",
                unsafe_allow_html=True,
            )
            st.caption(t("p2_od_note"))
