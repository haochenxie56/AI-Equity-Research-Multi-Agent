...
>
> 对齐基线：`main @ 1e389549b`（Phase 8B SectorRotationAgent docs closeout；最后一个 feature merge `fbf0cc41d`）。Active parity 套件：69 `test_reliability_*` GREEN；外加 agent framework 15 + MacroRegime 24 + MoneyFlow 34 + MarketStructure 44 + SectorRotation 34 + 8B-0 24 + 7B rotation 229。
> 更新日期 2026-06-24。**本版（v12）更新：MoneyFlowAgent / MarketStructureAgent / SectorRotationAgent 三个生产 agent 已落地（共四个 foundation agent 在产）；7D Block A / 8A / 8B-0 已收口；baseline 由 `a19b862` 推进至 `1e389549b`；roster 状态、数据源接入状态、两世界连接状态对齐 PROJECT_STATE。下一个 agent：ThemeIntelligenceAgent。**
>
...
| 状态 | 已上线，用户可见 | 框架已激活；**4 个 foundation agent 在产**，其余仍休眠 |
| 数据流 | 页面直接调 `llm_orchestrator`（研究层）；Cockpit 另附加 4 个 agent 钩子 | `AgentOutput` 内嵌校验后的 `AgentResult`；ToolResult → `EvidenceStore` → 受约束提示 → 校验 → JSONL 落盘 |
...
> 状态图例：✅ 已实现并在产 · ⏳ 下一个（STEP 0）· 📋 计划。Foundation Agent 规范 roster 共 11 个；行 1–4 已在产，行 5 为下一个。
...
ThemeIntelligenceAgent ⏳ 下一个（STEP 0）
  Code:  主题 Wave 激活状态（已有 theme_transmission：传导棒次 / cluster / 每票角色）
  LLM:   哪个 Wave 有最大非对称机会 → 具体 cluster 内哪类公司最先受益

CandidateScreeningAgent 📋 计划
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  当前：Investment Cockpit 已附加式接入 4 个 foundation agent 钩子（macro / money_flow /
        market_structure / sector_rotation），各为 key-gated、fail-closed、复用已算信号、绝不中断刷新
```
...
> **Phase 8B SectorRotationAgent ✅ 完成（`fbf0cc41d`，docs closeout `1e389549b`）→ 下一步：Phase 8B ThemeIntelligenceAgent（STEP 0 先行）**

已在产的 4 个 foundation agent：MacroRegimeAgent · MoneyFlowAgent · MarketStructureAgent · SectorRotationAgent。其后顺序：ThemeIntelligenceAgent → CandidateScreeningAgent → StockResearchAgent → TechnicalEntryAgent → SectorResearchAgent → RiskOverlayAgent（ValuationDebateAgent 归 Phase 8D）。
...
* 🔄 **Phase 8B** — 基础 Agent 实现（已在产 MacroRegime / MoneyFlow / MarketStructure / SectorRotation；下一个 ThemeIntelligence）
