...
> **本文件是什么**：一份"同步对齐后"的完整路线图，把"已完成进度"和"未来各阶段的设计初衷与目标"合并在一处，作为**防漂移的权威备份 + 跨会话迁移的压舱石**。归档于 `docs/Roadmap/`，与 `docs/ai_dev_state/PROJECT_STATE.md` / `docs/Memo/AI_Investment_OS_Master_Memo_v4.md`（Master Memo）/ README 并列。前一版 `docs/ROADMAP_v12.md` 原地保留为被取代的历史版本（append-only 文档纪律）。
>
> **为什么要有它**：过去几轮对齐发现，光记"要做什么"不够——容易在跨会话时把一个条目的**本质**记串。所以这份路线图的每个未来条目都固定写清五件事：**它是什么 / 为什么做 / 做到什么算完成 / 它绝不能变成什么（防漂移护栏）/ 谁来用它**。
>
> 对齐基线：`main @ f17f4e572`（Phase 8B CandidateScreeningAgent docs-hash follow-up；phase merge `e6c5be89a`，feature `b6daea492`；候选资格闸 enabler merge `f78ef606f`，feature `b1310ec1e`）。Active parity 套件：`test_reliability_*` GREEN；外加 agent framework 15 + MacroRegime 24 + MoneyFlow 34 + MarketStructure 44 + SectorRotation 34 + ThemeIntelligence 39 + **候选资格闸 18（87 断言）** + **CandidateScreening 104 断言** + 8B-0 24 + 7B rotation 229 + theme_baskets 157。
> 更新日期 2026-07-01。**本版更新（v13）：CandidateScreeningAgent 已落地（第六个生产 agent，v1 = 动量/RS-gap 单一策略）+ 确定性四态候选资格闸 enabler（`lib/candidate_eligibility.py`）已落地；下一个 = Degradation-Visibility Layer（Cockpit 聚合 banner，把每个 foundation agent 埋在 `supporting_data` 的降级词汇汇总呈现，并区分「防御式 fail-closed 降级=须保留、是数值防火墙」与「运行时可避免降级=从交易角度是 BUG」）。baseline 由 `6cc972e9a` 推进至 `f17f4e572`；roster 状态（六个在产）、UI 六钩子、两世界连接状态对齐 PROJECT_STATE。**
> 沿革（v12，2026-06-26，基线 `6cc972e9a`）：ThemeIntelligenceAgent 已落地（第五个生产 agent）；constituent_rs 扩展 + 标签 lift 已完成；下一个 CandidateScreeningAgent。baseline 由 `1e389549b` 推进至 `6cc972e9a`；roster 状态、数据源接入状态、两世界连接状态对齐 PROJECT_STATE。
...
* **enabler 先于 agent（已三次验证）**：确定性、LLM-free 的 enabler 单独先合并，再合并消费它的 agent 本体——`constituent_rs` 先于 ThemeIntelligenceAgent、`FragilityReading.offense_defense` 先于 SectorRotationAgent、`candidate_eligibility` 四态资格闸先于 CandidateScreeningAgent。enabler 承载数值防火墙 / provenance 那一侧，agent 本体承载 LLM 综合；两侧各自独立可审。
...
每个 phase 收口的 docs commit **必须同步**：PROJECT_STATE.md + CURRENT_TASK.md + phase doc + README.md。README 是一等交付物、**绝不滞后**。曾发生过一次 README 编辑误删五条不变量原则行 + 整段 v2 roadmap 的事故——须警惕任何"简化"README 的操作。**Master Memo / ROADMAP 采用版本后缀 + append-only**：新版（Memo v4 / ROADMAP v13）写入 `docs/Memo/` `docs/Roadmap/`，旧版（v3 / v12）原地保留为被取代历史，绝不删改。

### 0.4 Phase 0–5 地基扫描纪律

每个新 phase 开始前，STEP 0 勘察**必须**显式扫描 Phase 0–5 的遗留代码，检查是否有可复用的结构、数据类型或半成品实现。**有地基优先复用，无地基再新建。**

典型范例：Phase 7C 本可新建 tier/role 体系，但勘察发现 `lib/reliability/phase5_theme_intelligence.py` 已有完整的 `IndustryChainNode`、`ThemeCandidateRole` schema，节省了约 1,500 行已验合约的重复建设。CandidateScreeningAgent 亦复用了资格闸的 earnings-window 常量与 `_valuation_missing` provenance 检查，不重造。
...
* **边界升级（CandidateScreeningAgent 校正）**：边界不止"不出数字"。凡是能被确定性计算的**决策**（frontrunner、`no_clear_winner` 判定、slate 骨架），一律**代码决定、落为证据/`supporting_data`，LLM 不得覆写**。LLM 的活收窄为"解释哪些代码已算出的差异是决定性的" + 时间维度契合 + 失效条件。这是数值防火墙的最锋利形态——它防的是**决策**，不只是数字。
...
* **数值这一侧（只能更硬，永不松）**：估值、评分、阈值、技术指标、市场数据，永远是确定性 Python 算出来的。LLM **绝不**看着原始数字自己下数值结论。**确定性决策（frontrunner / `no_clear_winner`）同样归代码**，LLM 不得覆写。
...
* **排除而非降权**：某个估值锚不可信时，整条排除并打标，不引入"连续降权旋钮"。（CandidateScreeningAgent 的 tradability penny 守卫延续此纪律：`quality_capped` 是离散标签、只做注释——排除由资格闸负责——绝无连续乘子。）
* **绝不编造数字，也绝不编造字段**：LLM 永不发明估值/指标/评分/市场数据；无后备字段的维度（`short_crowding` / `options_structure` / `beta` / dollar-ADV）诚实标 `unavailable` 并排除出排序键，绝不杜撰数据源；`market_cap` 通常缺失时判 `unknown`，绝不假装 ample、绝不静默放行。
* **fail-closed 按 ticker 连接**：agent 跨两个生产者拼行时（如 CandidateScreeningAgent 用 `signals=` 把 CandidateSignal 字段拼到 OpportunityCard 上）**严格按 ticker 精确匹配、fail-closed**——未匹配/缺失/错票 → hard-unknown → rejected，绝不静默合格，绝不串扰他票。
...
* **降级词汇表 + 降级两分**：任何数据缺位都有具体原因标签，监控组件**绝不静默消失**。且须区分两类降级：**(1) 防御式 fail-closed 降级**（数据真的缺——是数值防火墙在起作用，**必须保留**）vs **(2) 运行时可避免降级**（key/tier/字段/缓存/网络没接好——从交易角度是 **BUG**）。Degradation-Visibility Layer 让两者都不可漏见（见§五）。
...
>
> **落地补记（CandidateScreeningAgent 实测）**：代码决定的确定性结构（frontrunner / `no_clear_winner` / slate 骨架）在 LLM 调用**前**放上 `AgentOutput.supporting_data`，由 `append_agent_output` 逐字节序列化——**不走** `extra="forbid"` 的 `AgentResult`（那会拒绝额外字段），也**不开第二个文件**。LLM 只在 findings 里给叙事判断 + 每票决定性因素，不选 primary、不改 `no_clear_winner`。
...
| **候选资格闸 enabler**（`lib/candidate_eligibility.py`，确定性四态闸，非 agent；18 tests / 87 断言） | ✅ 完成 | merge `f78ef606f`（feature `b1310ec1e`） |
| **Phase 8B CandidateScreeningAgent** — 第六个生产 agent（v1 动量/RS-gap 每主题相对筛选，104 断言） | ✅ 完成 | `f17f4e572`（feature `b6daea492`，merge `e6c5be89a`） |

### 架构现状快照（2026-07-01 更新）
...
| 状态 | 已上线，用户可见 | 框架已激活；**6 个 foundation agent 在产**，其余仍休眠 |
| 数据流 | 页面直接调 `llm_orchestrator`（研究层）；Cockpit 另附加 6 个 agent 钩子 | `AgentOutput` 内嵌校验后的 `AgentResult`；ToolResult → `EvidenceStore` → 受约束提示 → 校验 → JSONL 落盘 |
...
> 状态图例：✅ 已实现并在产 · ⏳ 下一个（STEP 0）· 📋 计划。Foundation Agent 规范 roster 共 11 个；**行 1–6 已在产**，其后为计划。
...
Processed Signals（代码，非 LLM）:
  ...
  candidate_eligibility（✅）        → 四态资格闸（eligible/conditional/ineligible/unknown）
                                      per (ticker, horizon)；六闸两层；数值防火墙 provenance 守卫
                                      （CandidateScreeningAgent 前置 enabler）
...
CandidateScreeningAgent ✅ 已实现（e6c5be89a；enabler f78ef606f）
  策略身份:  **v1 = 动量/RS-gap 单一策略**（主键 = 主题内每时间维度 RS 复合 gap）。
             未来 leader/high-beta 策略 = 独立 agent（如 LeaderScreeningAgent），
             与本 agent 并列列入 roster、绝非参数；v1 刻意无可插拔策略接口。
  前置 enabler:  lib/candidate_eligibility.py（确定性四态资格闸；六闸两层
                 HARD thesis/eps/valuation/event + SOFT liquidity/distribution；
                 horizon 非对称；数值防火墙 _forward_pe_is_usable provenance 守卫）
  Code:  对一个活跃主题的候选，逐个过资格闸 → 合格集上算确定性比较表
         A 维 relative_strength / valuation_elasticity / short_crowding(unavailable)
         B 维 theme_role / volume_confirmation / options_structure(unavailable) /
              catalyst_proximity / tradability
         横切 tradability quality_capped penny 守卫（exclude-not-down-weight）
         **代码决定** 每时间维度 frontrunner（short/mid 可不同）与 no_clear_winner
         （_RS_GAP_DECISIVE_PCT）；三 ToolResult；signal_basis 三值
         （signal_present / no_clear_winner / degraded_insufficient）
         short=coverage×决定性gap清晰度 / mid=coverage×mid清晰度 / long=0.0（defers）
  LLM:   只解释 frontrunner vs runner-up 哪些差异决定性、时间维度契合、失效条件；
         **不出任何数字、不选 primary、不改 no_clear_winner**；
         tone = "值得推进到 trade construction"，绝非"买 X"
  连接:  signals= 按 ticker fail-closed 拼 CandidateSignal 字段（卡上没有的 eps/valuation/entry-quality）
  落盘:  确定性 slate 骨架（primary/secondary/watch/rejected）落 AgentOutput.supporting_data 逐字节
         （无第二文件、不走 extra="forbid" 的 AgentResult）
  钩子:  Cockpit 附加式**每主题循环**（2–3 活跃主题，key-gated + fail-closed + 逐主题隔离，
         仅写 candidate_screening_agent_output；仅 session-state，本阶段无 widget）
  已知限制(P2): market_cap 生产环境通常缺失、dollar-ADV 无字段 → penny 守卫多退化为
                ALT_SIGNAL 代理（诚实但覆盖薄；下一阶段 Degradation-Visibility + 未来 live market-cap/ADV 通道 驱动）
...
StockResearchAgent 📋 计划
  ...
  注:    首版即内建 degradation-visibility 字段
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  当前：Investment Cockpit 已附加式接入 6 个 foundation agent 钩子（macro_regime_agent_output /
        money_flow_agent_output / market_structure_agent_output / sector_rotation_agent_output /
        theme_intelligence_agent_output / candidate_screening_agent_output——后者为按 theme_key 的 dict），
        各为 key-gated、fail-closed、复用已算信号、绝不中断刷新
  下一个 UI 增量：Degradation-Visibility Layer（聚合 banner，把各 agent 埋在 supporting_data
        的降级词汇汇总呈现，双语；区分防御式降级 vs 运行时可避免降级）
```
...
> **Phase 8B CandidateScreeningAgent ✅ 完成（feature `b6daea492`，merge `e6c5be89a`，docs `f17f4e572`；含四态资格闸 enabler `f78ef606f`）→ 下一步：Degradation-Visibility Layer**

已在产的 6 个 foundation agent：MacroRegimeAgent · MoneyFlowAgent · MarketStructureAgent · SectorRotationAgent · ThemeIntelligenceAgent · **CandidateScreeningAgent（v1 动量/RS-gap，含四态资格闸 enabler）**。**下一步 = Degradation-Visibility Layer**（Cockpit 聚合 banner），其后再推进剩余 foundation agent：StockResearchAgent → TechnicalEntryAgent → SectorResearchAgent → RiskOverlayAgent（首版即内建 degradation-visibility 字段；ValuationDebateAgent 归 Phase 8D）。
...
* 🔄 **Phase 8B** — 基础 Agent 实现（已在产 MacroRegime / MoneyFlow / MarketStructure / SectorRotation / ThemeIntelligence / **CandidateScreening（含四态资格闸 enabler）** 六个；下一个非 agent 增量 = Degradation-Visibility Layer；其后 StockResearch / TechnicalEntry / SectorResearch / RiskOverlay）
* 🔄 **Degradation-Visibility Layer（下一个）** — Cockpit 聚合 banner，surface 每个 foundation agent 的降级词汇；区分防御式 fail-closed 降级（须保留）vs 运行时可避免降级（BUG）（见§5.10）
...
| **绝不能变成什么** | Agent 不复述代码已算出的指标（那是 reporting，不是 synthesis）；LLM prompt 不允许输出泛论；`evidence_refs` 不允许为空；**代码决定的确定性结论（frontrunner / no_clear_winner）不允许被 LLM 覆写**。 |
...
**Phase 8B 实现顺序（8B-0 完成后）:**
    ...
    候选资格闸 enabler       ✅ 已完成（f78ef606f，lib/candidate_eligibility.py，非 agent）
    CandidateScreeningAgent  ✅ 已完成（e6c5be89a，v1 动量/RS-gap；消费上面的资格闸）
    ── Degradation-Visibility Layer  🔄 下一个（非 agent，Cockpit 聚合 banner，见§5.10）──
    StockResearchAgent       📋（首版即内建 degradation-visibility 字段）
    TechnicalEntryAgent      📋（消费 GEX walls 作为动态支撑阻力，见§5.9；首版即内建 degradation-visibility 字段）
    SectorResearchAgent      📋（首版即内建 degradation-visibility 字段）
    RiskOverlayAgent         📋（首版即内建 degradation-visibility 字段）
    （ValuationDebateAgent 归入 Phase 8D）

**CandidateScreeningAgent（第六个生产 agent）实现纪律（落地记录）:**

    v1 = 动量/RS-gap 单一策略（主键 = 主题内每时间维度 RS 复合 gap）；未来策略 = 独立 agent，无可插拔接口
    两阶段：① enabler lib/candidate_eligibility.py（确定性四态闸，先合并 f78ef606f）
            ② agent 本体（LLM 综合，e6c5be89a）
    代码决定 frontrunner + no_clear_winner；LLM 只解释决定性差异 + 时间维度契合 + 失效，绝不选 primary/改 no_clear_winner
    signals= 按 ticker fail-closed 拼 CandidateSignal 字段（未匹配 → hard-unknown → rejected）
    确定性 slate 骨架落 supporting_data 逐字节（无第二文件、不走 extra="forbid" AgentResult）
    下调维度诚实标 unavailable（short_crowding / options_structure / beta / dollar-ADV 无后备字段）
    校准债：_RS_GAP_DECISIVE_PCT=0.08 / _MCAP_AMPLE=10e9 / _VOL_CONFIRM_RATIO=1.2 / _COVERAGE_MIN=0.34
            （+ 资格闸的 _VALUATION_ELEVATED_PCT=0.70 / _VALUATION_PROHIBITED_PCT=0.85）
    已知限制 P2：penny 守卫多退化为 ALT_SIGNAL 代理（market_cap 常缺、dollar-ADV 无字段）
    104 断言全离线（§CSA-8 驱动真实 run_llm_agent 仅 stub _call_llm）；Codex APPROVE WITH FIXES（6 测试新增）→ APPROVE
...
**CandidateScreeningAgent 暴露的字段缺口（新增，2026-07-01）：**

    market_cap  — 生产环境通常缺失（仅 v1 FundamentalSignals shim 有）→ market_cap_tier 多为 unknown
    dollar-ADV  — 无字段 → tradability penny 守卫退化为 candidate_type == ALT_SIGNAL 代理
    short-interest / days-to-cover / % float — 无字段 → short_crowding 标 unavailable
    → 未来 live market-cap / ADV 通道是补齐 penny 守卫覆盖的前置
...
### 5.10 Degradation-Visibility Layer（下一个 · 非 agent）

| | |
|---|---|
| **它是什么** | Cockpit 的**聚合 banner**，把每个 foundation agent 埋在 `supporting_data` 里的降级词汇（`signal_basis` / `no_trade_reason` / `*_unavailable` 维 / 降级 coverage / `greeks_unavailable` / `vintage_mismatch` / `insufficient_data` …）统一汇总、双语呈现（`bi()` 纪律）。 |
| **为什么做** | CandidateScreeningAgent 等目前只把降级写进 `supporting_data`、**无任何可见 surface**——用户看不到某个主题为何"没有可推进候选"，也分不清是"数据真缺"还是"没接好"。 |
| **精确目的（锁定的设计初衷）** | **区分两类降级**：**(1) 防御式 fail-closed 降级**——数据真的缺（缺字段 / 无 key / 无每票信号），**这是数值防火墙在起作用，必须保留**；**(2) 运行时可避免降级**——key/tier/字段/缓存/网络没接好，从交易角度**是 BUG**。visibility layer 让两者都不可漏见。 |
| **做到什么算完成** | Cockpit 有一个聚合 banner，逐 agent 列出其降级项 + 类别（防御 vs 可避免）；双语；不改任何 agent 的确定性计算，只读 `supporting_data`。 |
| **绝不能变成什么** | 不得为了"好看"而抹掉防御式降级（那是防火墙）；不得把 (1) 误报成 (2)，也不得把 (2) 藏起来当作正常。 |
| **后续** | 待 8B roster 补齐后，定义**运行时零降级验收协议**（runtime zero-degradation acceptance protocol）：把 (2)=bug 制度化，并给 (1) 一份显式豁免清单。剩余 foundation agent（StockResearch / TechnicalEntry / SectorResearch / RiskOverlay）**首版即内建 degradation-visibility 字段**。 |
| **谁来用它** | 人工复核（一眼看清系统当天哪些信号真缺、哪些是没接好）；未来的验收协议与 Judgment Console。 |
...
| **候选池太杂、不知先深研谁** | 每主题相对筛选 + 四态资格闸 + 代码决定 frontrunner/no_clear_winner | CandidateScreeningAgent（8B） |

---

## 七、踩过的坑 / 决策范例（供未来会话参考）

1. **README 回退事故**：一次 README 编辑在旧基底上做，误删 5 条不变量原则行 + 整段 roadmap。处置：以权威 README 为基底，只 port 新原则，Codex 加回归守卫。教训：任何"简化"README 的操作都要 diff 全文、逐行核对。**同理适用于 Master Memo / ROADMAP 的版本升级：新版从旧版全文增量改，diff 逐行核对无原则/护栏/历史 phase 被误删。**
...
14. **附加式、key-gated、fail-closed 是把 agent 接进活页面的安全方式（Phase 8B 反复验证）**：所有 Cockpit agent 钩子都遵守同一模式——复用已算好的确定性信号（绝不二次 fetch/compute）、`_has_llm_api_key()` 门控、各自 try/except、只写自己那个 `*_agent_output` session key。一个无 key 或失败的 agent 是干净的 no-op，绝不中断刷新。**CandidateScreeningAgent 把它扩展为每主题循环**——2–3 个活跃主题各自 try/except 隔离，一个主题失败不拖累其他。

15. **注入而非重算，杜绝 vintage 分歧（MarketStructureAgent）**：脆弱度读数从 Cockpit Step 4 注入，agent 绝不自己调 `compute_market_fragility`——避免 banner 与 agent 之间出现二次计算与 vintage 漂移。同理 SectorRotationAgent 复用 Step 2 的 themes + 注入的 O/D reading，CandidateScreeningAgent 复用已算的 `_cards` / `_candidates` / `_themes_list`、绝不 re-rank。
...
17. **中性状态绝不可读成方向（SectorRotationAgent / CandidateScreeningAgent）**：`signal_basis` 三值含中性/等待态（SRA 的 `no_clear_leadership`、CSA 的 `no_clear_winner`）——数据齐但无决定性分离，prompt 显式禁止把它框成 bullish/bearish 或"卖出"。

18. **代码决定决策、LLM 只解释、绝不覆写（CandidateScreeningAgent，2026-07-01）**：frontrunner、`no_clear_winner`、slate 骨架全部代码算、在 LLM 调用前落为证据 / `supporting_data`。prompt 禁止 LLM 选 primary 或翻 `no_clear_winner`；LLM 的活收窄为"解释哪些代码已算差异是决定性的" + 时间维度契合 + 失效条件。这是数值防火墙的最锋利形态——它防的是**决策**，不只是数字。

19. **缺字段是诚实的 unavailable、绝不编造（CandidateScreeningAgent，2026-07-01）**：PREFLIGHT 发现 `short_crowding` / `options_structure` / `beta` / dollar-ADV 无后备字段——每个标 `unavailable` 并**排除出排序键**，`_sort_key` 专门测其读不到；`market_cap` 常缺则判 `unknown`（绝不假 ample、绝不静默放行）。recon-first PREFLIGHT 先把缺字段摊出来，把一次很可能的 REJECT 变成了 APPROVE-WITH-FIXES。

20. **fail-closed 按 ticker 连接（signals= 偏差，CandidateScreeningAgent，2026-07-01）**：草拟的 cards-only 签名够不到资格闸需要的 CandidateSignal 字段（eps/valuation/entry-quality），补一个显式 `signals=` kwarg 按 ticker 精确匹配、fail-closed（未匹配 → hard-unknown → rejected，绝不静默合格、绝不串扰他票）——而非放任一个静默默认让卡未经闸就通过。

21. **enabler 先于 agent（三次验证，2026-07-01 收束为纪律）**：确定性 LLM-free enabler 单独先合并——`constituent_rs` / `FragilityReading.offense_defense` / `candidate_eligibility` 四态闸——再合并消费它的 agent。enabler 承载数值防火墙 / provenance（如资格闸的 `_forward_pe_is_usable` 堵 `data_source["valuation"]="live"` 默认 0.5 泄漏），agent 本体承载 LLM 综合。两侧各自小、各自可审。

22. **降级两分：防御 vs 可避免（2026-07-01，Degradation-Visibility Layer 设计初衷）**：并非所有降级等价。**防御式 fail-closed 降级**（数据真缺）是数值防火墙在起作用、**必须保留**；**运行时可避免降级**（key/tier/字段/缓存/网络没接好）从交易角度**是 BUG**。两者今天都埋在 `supporting_data`，下一阶段的可见性层把两者都摊到台面上，之后再定"运行时零降级验收协议"把 (2)=bug 制度化、给 (1) 显式豁免清单。

---

*Numbers by deterministic code · Deterministic verdicts by code, explained (not overridden) by LLM · Evidence-bound judgment by LLM, confirmed by human · Review-only by design · AI Fund workflow, not a dashboard*
