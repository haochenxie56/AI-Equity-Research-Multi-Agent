...
Two independent **data-layer** changes shipped together as ONE closing commit, direct to
the main worktree (not a `--no-ff` branch merge). Both Codex-approved with discriminating
...
### ITEM 1 — bulk earnings-calendar fetch hoisted before the Track B fan-out (cold-cache 429 fix)

**Timing/wiring only — no logic, threshold, or computation change.** The single uncached
bulk Finnhub `fetch_earnings_reactions_calendar` call (one per refresh) ran LAST (Step 4 of
`pages/7_Investment_Cockpit.py::_run_refresh`, inside the fragility block). On a **cold
cache** the Step-3 Track B fan-out (`generate_candidates → run_track_b`, the FULL scan
universe × ~2 per-ticker Finnhub calls at 8 workers) exhausted Finnhub's free 60 req/min
budget BEFORE the bulk call, which then 429'd → `finnhub_unavailable`.

- **Fix:** the bulk call is **hoisted to fire ONCE, before Step 3**; its raw REPORTS are
  **replayed** into `compute_market_fragility` via `earnings_calendar_fn` (a closure
  returning the prefetched list). **Not** via `earnings_reactions=` — that path bypasses
  the Round-4 scan-universe pipeline (`_reaction_records` → `_earnings_components_asof` →
  `earnings_skipped` / `partial_frame_coverage`) and expects `next_session_return` records
  the raw calendar lacks, so it would change the computation. The replay keeps Step-4
  **byte-for-byte identical**; exactly one network call; no second call site.
- **Degrade preserved:** on early-fetch failure the **captured exception is re-raised** at
  Step 4 → identical `finnhub_unavailable` degrade (`good_news_sold=None`,
  `earnings_evaluated=0`); the refresh never crashes (fail-closed). Re-raising the captured
  exception (vs a fresh `RuntimeError`) also keeps pages/7 free of the lowercase `finnhub`
  vendor token the 5H/5N source guardrails forbid.
- **Ranking path unaffected:** `lib/opportunity_ranker.py` references none of
  `fetch_earnings_reactions_calendar` / `compute_market_fragility` / `earnings_calendar_fn`
  (structural guard) — the zero-network ranking contract is intact.
- **Tests** (`test_reliability_phase_7b_rotation_internals.py` §20, **211→219**): drives the
  REAL `_run_refresh` via AppTest recording call ORDER — 20.2 fetcher fires EXACTLY ONCE;
  20.3 BEFORE the Track B fan-out; 20.4 replay still drives the real reaction computation;
  20.5–20.7 an early failure does not crash and degrades to `finnhub_unavailable`; 20.8
  ranking zero-network. Mutations proven (reverted): duplicate call →
  `['earnings','earnings','track_b']` (20.2 RED); deferred/late fetch →
  `['track_b','earnings']` (20.3 + 20.6 RED). `signal_engine.py` untouched (the order spy
  patches from the test).

### ITEM 2 — FRED liquidity fetchers (SOFR / ON RRP / TGA / bank reserves), display-only, snapshot-excluded

Adds deterministic FRED money-market liquidity fetchers, surfaced on the Macro Dashboard
Liquidity tab. **Display-only; deliberately NOT written to the daily snapshot.** Mirrors
the existing `lib/macro_data.py` pattern — no new infrastructure, dependency, or key.
...
- **`lib/macro_data.py`:** new `LiquidityResult` dataclass (`sofr` / `on_rrp` / `tga` /
  ...
  helper (its own 429 retry/backoff; per-series failure isolation; fail-closed to fixture).
  Reuses the existing `FRED_API_KEY` (`dotenv` + `os.getenv`) — no new key, no `fredapi`.
- **Snapshot / regime isolation (load-bearing):** `fetch_liquidity` is **NOT** part of
  ...
  fields). Fetched **on demand** from the pages/8 render path only; source comments at the
  fetcher AND the render call site record the deliberate exclusion.
- **`pages/8_Macro_Dashboard.py`:** the existing `_render_live_liquidity` tab ADDS a
  money-market-liquidity group (four `_bignum`s + `_render_live_group_header` showing the
  live/fixture `data_source`); existing rates content unchanged. Five bilingual `t()` keys
  (`macro_live_grp_liquidity`, `macro_live_sofr` / `_on_rrp` / `_tga` / `_reserves`) added
  to BOTH locale tables in `ui_utils.py`.
- **Tests** (`test_reliability_phase_5o_macro_dashboard.py` §L, **+9**): L.1/L.1b/L.2/L.3
  fail-closed (forced exception + no-key → fixture, never raises/"live"); L.4–L.7 the
  SNAPSHOT-EXCLUSION guard (`MacroDataResult` fields disjoint from the series;
  `fetch_all_macro` doesn't call `fetch_liquidity`; `write_daily_snapshot` source carries no
  liquidity field; a REAL `write_daily_snapshot` `_meta` excludes all liquidity series);
  L.8 no-`fredapi`. Mutations proven (reverted): fixture tagged `live` → L.1/L.1b/L.2/L.3
  RED; `sofr` added to `MacroDataResult` → L.4 RED.

### Verification

`phase_7b` **219/219**; `phase_5o` **738/3** (the 3 reds are pre-existing `url_pathname`
AppTest-harness errors, identical to HEAD); 5H **179/24** and 5N **545/104** — identical to
HEAD baseline, **zero new failures** from the shared `ui_utils.py` / `pages/8` edits.
`README.md` NOT re-touched (the segment-1 principle correction shipped @ `f99ed2f`). No
`macro_regime.py` / `opportunity_ranker.py` / snapshot-assembly changes.
...
Round 1 unifies the fair-value anchor onto a **single producer**, makes the
sell-side analyst input **structured**, and stamps every card with the producer
**epoch** so the Trading Desk, Equity page, and Cockpit can never silently
diverge. **All deterministic; no LLM invents anchors.** Phase doc
`docs/reliability_anchor_intel_v2.md`; suite
`scripts/test_reliability_phase_6c_v3_entry_v4.py` **90/90** (52 → 73 after fix
round 1 → 87 after fix round 2 → 90 after the R3 real-path case); canonical sweep
green. Review **APPROVED at 9e53f04**; round 1 is CLOSED. **Rounds v2.2–v2.5 remain
pending** (future scope, not started).
...
+ allows network; ranker = network-free; refresh = no-poison) before
implementing.*

- **U1 — single fair-value producer.** `order_advisor._gather_technicals` now
  computes the anchor via `lib.equity_valuation.compute_app_fair_value`
  (`AppFairValue`) instead of the retired `lib.valuation_anchor`. Production has
  ONE producer + ONE `st.cache_data` entry per ticker; Trading Desk, Equity page,
  and Cockpit render from the same producer/epoch. Entry-strategy FORMULAS
  unchanged (×0.90 high, ×0.85 medium); only the anchor inputs migrated.
  `conservative_anchor` is tier-dependent (high = `fair_value_mid`, medium =
  `analyst_target`, low/suppressed = None) — using `fair_value_low` here would
  double-discount against the ×0.90 MoS. `lib/valuation_anchor.py` is deprecated
  (retired from production, kept for compatibility).
- **U2 — structured analyst anchor + pool-dispersion gate.** `AppFairValue` now
  carries the sell-side target distribution as a structured `analyst_pool`
  {median, mean, high, low, n, as_of}; the median remains the central estimate
  entering the blend. A pool-dispersion confidence gate (`pool_dispersion =
  (high − low)/median`, threshold 1.0×, min n = 3) caps confidence at low and
  emits `CAVEAT_ANALYST_POOL_DISPERSED` when it fires (confidence-only; the blend
  still proceeds; the inter-method dispersion gate still runs last).
- **U3 — epoch stamping + single-source-per-card.** `PriceLevelResult` carries the
  `AppFairValue` epoch (`fair_value_computed_at`); when an external/session band
  drives the card, confidence AND `conservative_anchor` AND the epoch all come
  from THAT band (no mixed-source read against the local instance).

### Fix rounds (hardening, all APPROVED at 9e53f04)

- **F-round** — true single cached producer (page-path fetcher threaded so one
  cache entry/epoch is shared; first-writer-wins parity proven on the REAL Equity
  vs Trading-Desk paths); external band made single-source; the misleading
  `analyst_proxy` `fair_value_source` token renamed to `app_fair_value` (C5).
- **X-round** — `compute_price_levels(allow_fetch=False)` makes the ranker /
  Cockpit-refresh path **structurally network-free** (the live producer is reached
  only on page paths; a cold cache degrades honestly to
  `fair_value_source="anchor_not_cached"` + `fair_value_anchor=None`); Cockpit
  on-demand equity-research passes the cyclical fetcher so no first writer can
  poison the shared cache entry (Option B); the external-band single-source path is
  gated so a healthy external band is never degraded by an irreconcilable local
  instance (the inverse leak).
- **R3** — on the cold / missing-OHLCV `anchor_not_cached` path the fixture's
  back-computed `cp×0.85` scalar is explicitly cleared to None, so the
  honest-degrade contract holds (no fabricated anchor rides through); proven on the
  REAL transitive ranker path (`rank_opportunities → compute_price_levels →
  _gather_technicals`), with discrimination verified (reverting the one-line clear
  re-fails the case).
...
**Added:** `.gitattributes` (enforce LF). **Tests:** `phase_6c_v3_entry_v4`,
`phase_6c_v2_entry_strategy`, `phase_6c_trading_desk`, `phase_6c_b_cockpit_rebuild`,
`valuation_router`. **Docs:** `docs/reliability_anchor_intel_v2.md` (new), state docs.
...
project_driven / cyclical from deterministic rules over the SAME `tk.info` dict
the valuation path already fetches (no new network). ONE visible
`CLASSIFIER_CONFIG` block (growth/margin/FCF/volatility/size thresholds) + sector
& industry hint lists; auditable `fired_rules`; `clear`/`borderline` confidence
(borderline routes to the default mature menu via `select_method_menu`). Fixture
...
gate (3.0×) still runs LAST** on the menu-produced set.

**Task 3 — Growth-profile peers** (`lib/valuation_router.py`):
`match_growth_profile_peers` = same sector AND growth band (<10/10–25/>25) AND
size band (large ≥$10B/mid ≥$2B), falling back to sector-only with
`peer_basis="sector_fallback"` when < `min_peers` (default 4). `pages/4` collects
peer `info` in the existing peer loop and passes it to the routed valuation (no
new fan-out); the cached/Cockpit path uses `peers=None` → sector fallback.

**Task 4 — Integration**: `AppFairValue` carries `company_type` +
`routing_rationale` + `methods_used` + `excluded_anchors` + the extra anchors +
`peer_basis` + `backlog_note`. **Anchor cache schema 1 → 2** (entry adds
company_type/peer_basis; the version guard migrates old entries to empty). `pages/4`
shows the company-type badge + per-anchor method labels + excluded anchors;
`lib/financial_tab.py` shows an honest "DCF not the primary method for this type"
note; Cockpit needs no change beyond the cache version bump (verified — the LONG
consumer reads only the band + blend_state). KTOS acceptance: relative $3.80 vs
analyst $30 (7.89× → irreconcilable, $0 band) → project_driven EV/EBITDA $23.46 +
analyst $30 (1.28× → **blended $19.94 / $25.91 / $31.50**).

**Created:** `lib/valuation_router.py`,
...
`scripts/test_reliability_valuation_stopbleed.py` (version-bump-aware), state docs.
