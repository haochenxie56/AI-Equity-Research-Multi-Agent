...
| **CandidateScreeningAgent**（v1：动量/RS-gap 策略 / momentum·RS-gap）| 机会发现 Opportunity | `opportunity_ranker` + `candidate_generator` / `signal_engine` + `candidate_eligibility`（四态资格闸）| ✅ 已实现 `e6c5be89a`（本体；资格闸 enabler `f78ef606f`）|
